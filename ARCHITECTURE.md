...
8. **Shared Commitments, Milestone SC1** — *built; backend tested (migration 0020 + `170_shared_commitments.sql`, both CI layouts); not yet on the hosted project; device verification pending.* Doing it together (NORTHSTAR §46): create with friends, invitations with real consent states, acceptance through the invitee's own Deal producing an ordinary personal commitment (own Gate, own rules, hardening from own acceptance — EarnedKit deliberately unchanged), the roster with self-reported progress lines, block/friendship rules. Design: `docs/shared-commitments.md`.
