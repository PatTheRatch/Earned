...
7. **Social Accountability, Milestones S1 + S2** — *built + tested.* S1: profiles (handle, avatar, optional city), mutual friendships with block semantics, and the Social tab. S2: per-commitment sharing (default Private), the bounded activity shelf with its 30-day retention, and streak presentation to the settled two-figure semantics (`EarnedState.socialStreaks`). Design: `docs/social-architecture.md`; check-in/inactivity sharing is the deliberately later S3.
