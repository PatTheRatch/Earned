...
1. ~~**EarnedKit core**~~ — *built + tested.* Event ledger, gate engine (hydration + exercise), hardening/monotonicity, debt, override state machine, enforcement-integrity semantics, plans, verification tiers. 175 tests green on Linux and macOS, including a golden fixture for every ledger schema version that can be on a phone (v1–v6).
...
3. **Enforcement** — *built; not device-verified.* FamilyControls authorization, `FamilyActivityPicker` app selection, and `ManagedSettingsStore` shields are real (`app/Earned/Enforcement/`); shields fail closed. The `DeviceActivityMonitor` extension is built (`app/EarnedMonitor/`): the app writes a `ShieldPlan` into the App Group, the system wakes the extension at each scheduled deadline, and `intervalDidStart` applies the shield without the app running. `intervalDidEnd` is deliberately empty — only the app knows whether the commitment was met. **Whether iOS actually wakes it has never been observed on hardware**, and it cannot be: the simulator does not run DeviceActivity schedules and CI signs nothing. Until a phone says otherwise this is the single largest unknown in the product (`docs/beta-readiness.md` B‑2). Still missing: a `ShieldConfiguration` extension (blocked apps show Apple's default shield, not `NICE TRY.`).
...
7. **Social Accountability, Milestones S1–S3** — *built + tested; backend deployed (migrations 0013–0018 applied and verified on the hosted project, cron scheduled — `docs/deployment.md` §7–8). Device verification pending.* S1: profiles (handle, avatar, optional city), mutual friendships with block semantics, and the Social tab. S2: per-commitment sharing (default Private), the bounded activity shelf with its 30-day retention, and streak presentation to the settled two-figure semantics (`EarnedState.socialStreaks`). S3: opt-in check-in sharing and the hasn't-checked-in surface — one stored fact, whole days only, surfaced past 72 hours, facts never motive. Design: `docs/social-architecture.md`.
8. **Shared Commitments, Milestone SC1 + follow-ups** — *built; backend tested (migrations 0021–0022 + `180`/`190` test files, both CI layouts) and deployed (applied to the hosted project 1 September 2026, with `announce-shared-starts` and `purge-shared-commitments` scheduled — `docs/deployment.md`); device and multi-user verification pending.* Doing it together (NORTHSTAR §46): create with friends, invitations with real consent states, acceptance through the invitee's own Deal producing an ordinary personal commitment (own Gate, own rules, hardening from own acceptance), the roster with self-reported progress lines, block/friendship rules. The settled follow-ups: `CompletionMetric.sessionCount` (ledger schema v6 — the one EarnedKit change this area has needed), creator deletion orphaning agreements rather than dissolving them, roster moments as Recent-shelf events, and push groundwork under the revised rule (no social-engagement push; commitment-relevant push allowed — outbox + token registry built, APNs delivery a deployment step). Design: `docs/shared-commitments.md`.

The Xcode project is **generated** from `app/project.yml` and not committed, so there are no
project-file merge conflicts. Screen Time, HealthKit and Sign in with Apple are all declared
now, in two checked-in entitlement files that generation must not touch — `app/project.yml`
carries the long comment explaining why, and CI fails if `xcodegen generate` rewrites either
of them.

Family Controls (Distribution) is required for **both** bundle ids before any TestFlight
build, and Apple reviews each by hand (`docs/beta-readiness.md` B‑1).
